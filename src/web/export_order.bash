...
curl -X GET "https://${DATA[3]}:${DATA[4]}@${DATA[2]}/admin/api/2020-07/orders.json?status=open&limit=250&fulfillment_status=unfulfilled" | python -m json.tool | jq -r ".orders[].id" | cat >> /root/shopify.${DATA[0]}${DATA[1]}.orders.unfufilled.listID

#2. Count unfulfilled
curl -X GET "https://${DATA[3]}:${DATA[4]}@${DATA[2]}/admin/api/2020-07/orders/count.json?fulfillment_status=unfulfilled" | python -m json.tool | jq -r ".count" | cat > /root/shopify.${DATA[0]}${DATA[1]}.orders.count 
...
curl -X GET "https://${DATA[3]}:${DATA[4]}@${DATA[2]}/admin/api/2020-07/orders.json?ids=${orderID}" | python -m json.tool | jq -r ".orders[].line_items[].product_id" | cat > /root/shopify.${DATA[0]}${DATA[1]}.orders.${orderID}.qttProducts
...
#REMOVE TRASH
rm -rf /root/woocommerce* /root/shopify*
clear
