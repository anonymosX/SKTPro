...
printf " ============================================\n"
	...
	printf "`sed -n '1p' /etc/skt.d/data/namesilo/namesilo_$NUMBER`" | cat > /root/$DOMAIN/api_ns.txt
	yes | cp -rf /root/$DOMAIN/api_ns.txt /etc/skt.d/data/$DOMAIN/api_ns.txt
	curl -X POST "https://www.namesilo.com/api/changeNameServers?version=1&type=xml&key=`sed -n '1p' /etc/skt.d/data/namesilo/namesilo_$NUMBER`&domain=$DOMAIN&ns1=`sed -n '1p' /root/$DOMAIN/ns_cf.txt `&ns2=`sed -n '2p' /root/$DOMAIN/ns_cf.txt `&ns3="
